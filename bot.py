...
async def parrot(ctx):
    ...
    parrot = not parrot
    if parrot:
        await ctx.send("Now parroting.")
    else:
        await ctx.send("No longer parroting.")


@client.event
async def on_message(message):
    global parrot
    if str(message.author) == "Duncanator02#5596" and parrot:
        await message.channel.send(message.content)
    else:
        await client.process_commands(message)
    """ 
        except:
                await message.channel.send("Invalid command.",delete_after=25)
        """
...
async def on_ready():
    ...
    for guild in client.guilds:
        # create_guild(str(guild.id))
        # set_guild_name(str(guild.id), str(guild.name))
        clearQueue(str(guild.id))
    loadAllData()
    # await client.change_presence(activity=discord.Game(name="Tinder for Overwatch"))
    print("bot is ready")
    # await posture()
...
async def posture():
    global bot_owner
    tony = client.get_user(364167513971621890)
    await bot_owner.send("Posture check", delete_after=3)
    await tony.send("Posture check uwu", delete_after=90)
...
async def fuck_you(ctx):
    try:
        ...
        pass
    vip_list = getVipList(str(ctx.message.guild.id))
    if str(ctx.message.author.id) in vip_list:
        for i in range(5):
            await ctx.send(ctx.message.mentions[0].mention,
                           delete_after=1)
...
async def scrim(ctx):
    ...
    for i in ctx.message.guild.roles:
        if str(i) == "Darkmode":
            darkmode = i
    vip_list = getVipList(str(ctx.message.guild.id))
    if str(ctx.message.author.id) in vip_list and ctx.message.guild.name == "RESURGE Darkmode Esports":
        await ctx.send(darkmode.mention + " scrim at 5 today. Remember to come on 30m before for warmup.")
...
async def gay(ctx):
        ...
        if str(ctx.message.author) == "Duncanator02#5596":
                await ctx.send(ctx.message.author.mention + " is p cute ngl.")
